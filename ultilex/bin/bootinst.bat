...
echo =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
echo.
echo This installer will setup disk %DISK%: to boot only ULTILEX.
echo.
...
:setupNT
\ultilex\stuff\syslinux\syslinux.exe -maf -d \ultilex\stuff\syslinux %DISK%:
goto setupDone

:setup95
\ultilex\stuff\syslinux\syslinux.com -maf -d \ultilex\stuff\syslinux %DISK%:
...
:readOnly
echo You're starting ULTILEX installer from a read-only media, this will not work.
goto pauseit
