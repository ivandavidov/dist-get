...
clear
echo "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-"
echo "                      Welcome to ULTILEX boot installer                        "
echo "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-"
echo
echo "This installer will setup disk $TARGET to boot only ULTILEX."
if [ "$MBR" != "$TARGET" ]; then
   echo
   ...
   echo "anymore. Only ULTILEX will boot from this device. Be careful!"
fi
echo
...
if [ "$MBR" != "$TARGET" ]; then
   ...
   ./ultilex/stuff/syslinux/lilo -S /dev/null -M $MBR ext # this must be here to support -A for extended partitions
   echo "Activating partition $TARGET..."
   ./ultilex/stuff/syslinux/lilo -S /dev/null -A $MBR $NUM
   echo "Updating MBR on $MBR..." # this must be here because LILO mbr is bad. mbr.bin is from syslinux
   cat ./ultilex/stuff/syslinux/mbr.bin > $MBR
fi

echo "Setting up boot record for $TARGET..."
./ultilex/stuff/syslinux/syslinux -d ultilex/stuff/syslinux $TARGET
